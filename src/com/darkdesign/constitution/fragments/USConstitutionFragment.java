...
public class USConstitutionFragment extends SListFragment {

	private static USConstitutionFragment instance;
	...
	public static USConstitutionFragment getInstance() {
		if (instance == null) {
			return new USConstitutionFragment();
		}
		return instance;
	}

	public USConstitutionFragment() {
		instance = this;
	}
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		if (isABSSupport()) {
			...
			String[] listObjects = new String[Globals.constitutionData.size()];
	...
	public class MyCustomAdapter extends ArrayAdapter<String> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			String title = "";
			if (Globals.constitutionData.get(position).getTitle().contains(":")) {
				title = Globals.constitutionData
						.get(position)
						.getTitle()
						.substring(
								0,
								Globals.constitutionData.get(position)
										.getTitle().indexOf(":"));
			} else {
				title = Globals.constitutionData.get(position).getTitle();
			}

			textviewTitle.setText(title);

			TextView textviewSubtitle = (TextView) row.findViewById(R.id.title);
			textviewSubtitle.setText(Globals.constitutionData.get(position)
					.getSubtitle());
